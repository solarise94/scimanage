...
import { reopenComplaint } from "@/lib/crm/complaints";
...
export async function POST(
  ...
  try {
    ...
    if (msg === "INVALID_STATUS") return NextResponse.json({ error: "当前状态不可重新打开" }, { status: 400 });
    return NextResponse.json({ error: "重新打开客诉失败" }, { status: 500 });
  }
}
