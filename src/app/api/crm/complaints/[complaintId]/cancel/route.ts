...
import { cancelComplaint } from "@/lib/crm/complaints";
...
export async function POST(
  ...
  try {
    const complaint = await cancelComplaint({
      complaintId,
    ...
    if (msg === "INVALID_STATUS") return NextResponse.json({ error: "当前状态不可取消" }, { status: 400 });
    return NextResponse.json({ error: "取消客诉失败" }, { status: 500 });
  }
}
